...
// Adds bi-directional edge
func (g *graph) AddBiDirEdge(origin, destiny string, weight int) {
	g.nodes[origin] = append(g.nodes[origin], edge{node: destiny, weight: weight})
...
func (g *graph) GetPath(origin, destiny string) (int, []string) {
	...
	for len(*h.values) > 0 {
		...
		for _, e := range g.getEdges(node) {
			if !visited[e.node] {
				// Calculate total weight spent so far plus cost and the path of getting to this node
